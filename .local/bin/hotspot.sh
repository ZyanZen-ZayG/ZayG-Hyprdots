...
write_config() {
  ...
  #   However, the /etc/hotspot.conf
  #   location is excepted.
  if [[ "$STORE_CONFIG" != "/etc/hotspot.conf" && $PKEXEC_UID ]]; then
    ...
    # need to run hotspot directly with sudo in order to write to, say, /etc/hotspot2.conf)
    touch "$STORE_CONFIG"
...
if [[ $(id -u) -ne 0 ]]; then
  echo "hotspot must be run as root." >&2
  exit 1
fi
